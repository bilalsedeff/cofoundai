...
async def startup_event():
    ...
    try:
        ...
        agent_configs = [{
            ...
            "system_prompt":
            """You are a vision analyst agent. Your role is to:
...
4. Generate clear, actionable project briefs""",
            ...
            "system_prompt":
            """You are a blueprint generator agent. Your role is to:
...
4. Create initial architectural recommendations""",
        ...
        orchestrator = AgenticGraph(project_id="system", agents=agents)
...
async def dream_phase(request: DreamRequest):
    ...
    try:
        ...
        Selected Tags: {', '.join(request.tags or [])}
        Goal: {request.goal}
        Tech Preferences: {', '.join(request.tech_preferences or [])}
        ...
        total_tokens = len(dream_prompt.split()) + len(
            response_content.split())
        estimated_cost = total_tokens * 0.00002  # Rough estimate

        return DreamResponse(initial_brief=response_content,
                             extracted_tags=(request.tags or []) +
                             ["AI-Generated", "CoFound.ai"],
                             cost_estimate={
                                 "tokens": total_tokens,
        ...
        raise HTTPException(status_code=500,
                            detail=f"Dream phase failed: {str(e)}")
...
async def maturation_phase(request: MaturationRequest):
    ...
    try:
        ...
        artifacts = [{
            ...
            "generated",
            "content":
            "Business Requirements Document has been refined based on stakeholder input."
        }, {
            "name":
            ...
            "generated",
            "content":
            "Detailed functional requirements have been documented."
        }]
        ...
        return MaturationResponse(artifacts=artifacts,
        ...
        raise HTTPException(status_code=500,
                            detail=f"Maturation phase failed: {str(e)}")
...
async def assemble_phase(request: AssembleRequest):
    ...
    try:
        ...
        return AssembleResponse(team_composition=team_composition,
                                estimated_cost={
        ...
        raise HTTPException(status_code=500,
                            detail=f"Assemble phase failed: {str(e)}")
